		 ...
		 const char* account)
{
  ...
  if (!str_copys(&creds[1].value, sasl_domain))
    return 0;
...
static int authenticate_free(int count, struct cvm_credential* creds)
{
  int result;
  result = cvm_client_authenticate(sasl_mech_cvm, count, creds);
  free_creds(count, creds);
  return result;
}

int sasl_authenticate_plain(const char* account, const char* password)
{
  struct cvm_credential creds[3];
  if (!setup(3, creds, account))
    return SASL_TEMP_FAIL;
  creds[2].type = CVM_CRED_PASSWORD;
  if (!str_copys(&creds[2].value, password))
    return SASL_TEMP_FAIL;
  switch (authenticate_free(3, creds)) {
...
int sasl_authenticate_cram(const char* account, const char* cram_type,
			   const str* challenge, const str* response)
{
  struct cvm_credential creds[5];
  if (!setup(5, creds, account))
    return SASL_TEMP_FAIL;
  ...
  if (!str_copys(&creds[4].value, cram_type))
    return SASL_TEMP_FAIL;
  switch (authenticate_free(5, creds)) {
  case 0: return SASL_AUTH_OK;
  case CVME_PERMFAIL: return SASL_AUTH_FAILED;
  default: return SASL_TEMP_FAIL;
  }
